...
func (c *Client) CleanUserService(ctx context.Context, path string, remove *bool) (*http.Response, error) {
	req, err := c.NewCleanUserServiceRequest(ctx, path, remove)
	if err != nil {
		return nil, err
	}
...
func (c *Client) NewCleanUserServiceRequest(ctx context.Context, path string, remove *bool) (*http.Request, error) {
	scheme := c.Scheme
	if scheme == "" {
		scheme = "http"
	}
	...
	values := u.Query()
	if remove != nil {
		tmp128 := strconv.FormatBool(*remove)
		values.Set("remove", tmp128)
	}
